...
impl JobControl {
    ...
    pub fn checkpoint(&self) -> Result<(), DuplicateError> {
        if self.cancelled.load(Ordering::SeqCst) {
            return Err(DuplicateError::ScanCancelled);
        }
        let mut paused = self
            .paused
            .lock()
            .map_err(|_| DuplicateError::ScanCancelled)?;
        while *paused {
            paused = self
                .pause_signal
                .wait(paused)
...
pub static JOBS: Lazy<Mutex<HashMap<String, Arc<JobControl>>>> =
...
pub fn control(job_id: &str) -> Option<Arc<JobControl>> {
