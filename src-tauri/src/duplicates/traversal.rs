use crate::duplicates::{
    contracts::DuplicateScanRequest, errors::DuplicateError, jobs::JobControl,
};
...
fn identity(_metadata: &fs::Metadata, canonical: &Path) -> (String, u64) {
    ...
    if handle == INVALID_HANDLE_VALUE {
        return (format!("path:{}", canonical.display()), 1);
    }
    ...
    (
        format!("win:{}:{}", info.dwVolumeSerialNumber, index),
        u64::from(info.nNumberOfLinks.max(1)),
    )
}
...
pub fn is_protected_path(path: &Path) -> bool {
    ...
    for variable in ["WINDIR", "ProgramFiles", "ProgramFiles(x86)", "ProgramData"] {
        if let Ok(value) = std::env::var(variable) {
            protected_roots.push(normalize_path(Path::new(&value)));
        }
    }
...
pub fn collect_files(
    ...
    control: &JobControl,
) -> Result<TraversalResult, DuplicateError> {
    if request.paths.is_empty() {
        return Err(DuplicateError::InvalidScanSource(
            "At least one scan source is required.".into(),
        ));
    }
    let canonical_exclusions = request
        .excluded_paths
        .iter()
    ...
    for source in &request.paths {
        ...
        if !source_path.exists() {
            result
                .errors
                .push(format!("source_not_found: {}", source_path.display()));
            continue;
        }
        let canonical_source = dunce::canonicalize(&source_path)
            .map_err(|error| DuplicateError::InvalidScanSource(format!("{source}: {error}")))?;
        if is_protected_path(&canonical_source) {
            return Err(DuplicateError::ProtectedPathSelected(
                canonical_source.display().to_string(),
            ));
        }
        if !visited_roots.insert(canonical_source.clone()) {
            continue;
        }
        let walker = if canonical_source.is_file() {
            WalkDir::new(&canonical_source).max_depth(0)
        } else if request.include_subfolders {
            WalkDir::new(&canonical_source)
        } else {
            WalkDir::new(&canonical_source).max_depth(1)
        };
        for entry_result in walker.follow_links(false) {
            ...
            let entry = match entry_result {
                Ok(value) => value,
                Err(error) => {
                    result.errors.push(format!("walk_error: {error}"));
                    continue;
                }
            };
            ...
            if canonical_exclusions
                ...
                || !extension_allowed(&canonical, request)
            {
                continue;
            }
            ...
            result.files.push(FileCandidate {
                path: entry.path().to_path_buf(),
                canonical_path: canonical.clone(),
                name,
                extension,
                size_bytes: size,
                modified_time: time_to_rfc3339(metadata.modified()),
                created_time: time_to_rfc3339(metadata.created()),
                mime_type,
                file_identity,
                hard_link_count,
                protected_path: is_protected_path(&canonical),
            });
        }
    }
    if result.files.is_empty() && result.errors.len() == request.paths.len() {
        return Err(DuplicateError::SourceAccessDenied(result.errors.join("; ")));
    }
    Ok(result)
}
