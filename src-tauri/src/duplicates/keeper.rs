use crate::duplicates::{
    contracts::{KeeperGroupPlan, KeeperPlanRequest, KeeperPlanResult},
...
pub fn build_plan(request: KeeperPlanRequest) -> KeeperPlanResult {
    ...
    let configured_protected = request
        .rules
        .protected_paths
        .iter()
    ...
    for group in request.groups {
        let mut eligible = group
            ...
            .filter(|file| {
                ...
                !configured_protected
                    .iter()
                    .any(|protected| normalized.starts_with(protected))
            })
            .collect::<Vec<_>>();

        if eligible.is_empty() {
            blocked_group_ids.push(group.group_id.clone());
            plans.push(KeeperGroupPlan {
                group_id: group.group_id,
                keeper_file_id: String::new(),
                selected_file_ids: Vec::new(),
                reason: "No eligible keeper exists outside protected paths.".into(),
                blocked: true,
                warnings: vec!["keeper_missing".into()],
            });
            continue;
        }
        ...
        eligible.sort_by(|left, right| {
            if let Some(preferred) = &request.rules.preferred_directory {
                ...
                let left_preferred =
                    normalize_path(Path::new(&left.canonical_path)).starts_with(&normalized);
                let right_preferred =
                    normalize_path(Path::new(&right.canonical_path)).starts_with(&normalized);
        ...
        let selected = if request.rules.auto_select_non_keepers && group.actionable {
            group
                ...
                .filter(|file| {
                    file.id != keeper.id
                        && !file.protected_path
                        && !file.is_hard_link_alias
                        && !configured_protected.iter().any(|protected| {
                            normalize_path(Path::new(&file.canonical_path)).starts_with(protected)
                        })
                })
        ...
        plans.push(KeeperGroupPlan {
            group_id: group.group_id,
            keeper_file_id: keeper.id.clone(),
            selected_file_ids: selected,
            reason: format!(
                "Keeper selected using date={:?}, path={:?}, preferredDirectory={:?}.",
                request.rules.prefer_date,
                request.rules.prefer_path,
                request.rules.preferred_directory
            ),
            blocked: false,
            warnings: Vec::new(),
        });
