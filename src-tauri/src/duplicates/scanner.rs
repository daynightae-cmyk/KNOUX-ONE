use crate::duplicates::{
    ...
    hashing::{full_blake3, partial_blake3, verify_unchanged},
    jobs::JobControl,
    traversal::{collect_files, FileCandidate},
};
use chrono::Utc;
use std::{
...
fn emit_progress(
    ...
    callback(DuplicateJobProgress {
        job_id: job_id.to_string(),
        operation_id: operation_id.to_string(),
        phase: phase.to_string(),
        mode: if total_files.is_some() {
            "determinate".into()
        } else {
            "indeterminate".into()
        },
...
pub fn scan_exact(
    ...
    let candidate_size_groups: Vec<Vec<FileCandidate>> = by_size
        .into_values()
    ...
    for group in candidate_size_groups {
        for candidate in group {
            ...
            match partial_blake3(&candidate.canonical_path, candidate.size_bytes) {
                Ok(hash) => {
                    partial_groups
                        .entry((candidate.size_bytes, hash.clone()))
                        .or_default()
                        .push((candidate.clone(), hash));
                }
    ...
    let partial_candidates: Vec<Vec<(FileCandidate, String)>> = partial_groups
        .into_values()
        .filter(|group| group.len() > 1)
        .collect();
    if mode == "fast_partial" {
        ...
        for group in partial_candidates {
            ...
            let files = group
                .into_iter()
                .map(|(candidate, partial)| {
                    build_file_item(&candidate, String::new(), Some(partial), false)
                })
                .collect::<Vec<_>>();
            groups.push(DuplicateGroup {
                group_id: Uuid::new_v4().to_string(),
                mode: mode.into(),
                category: category_for_extension(&files[0].extension).into(),
                wasted_size_bytes: 0,
                common_hash,
                proof_status: "candidate".into(),
                confidence: 0.55,
                actionable: false,
        ...
        return Ok(DuplicateScanResult {
            ...
            summary: DuplicateScanSummary {
                ...
                duplicate_groups_found: candidate_groups_count,
            ...
            match full_blake3(&candidate.canonical_path) {
                Ok(hash) => {
                    ...
                    full_groups
                        .entry(hash)
                        .or_default()
                        .push((candidate.clone(), partial_hash));
                }
                Err(error) => errors.push(error.to_string()),
            }
            verified_files += 1;
    ...
    for (common_hash, members) in full_groups {
        ...
        if files.iter().any(|file| file.hard_link_count > 1) {
            warnings.push(
                "Hard-link aliases were detected and are not counted as reclaimable storage."
                    .into(),
            );
        }
        groups.push(DuplicateGroup {
            ...
            wasted_size_bytes,
            common_hash,
            proof_status: if only_hard_links {
                "hard_link_aliases".into()
            } else {
                "verified_exact".into()
            },
            confidence: 1.0,
...
pub fn with_extensions(
    ...
    filtered.extensions = extensions.iter().map(|value| value.to_string()).collect();
    filtered
}

pub fn exact_for_extensions(
    operation_id: &str,
    job_id: &str,
    mode: &str,
    request: &DuplicateScanRequest,
...
mod tests {
    ...
    use crate::duplicates::{
        contracts::{DuplicateJobProgress, DuplicateScanRequest},
        jobs::JobControl,
    };
    use std::fs;
    fn request(path: &std::path::Path) -> DuplicateScanRequest {
        DuplicateScanRequest {
