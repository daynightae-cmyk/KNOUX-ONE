use crate::duplicates::{
    contracts::{FolderComparison, FolderComparisonRequest, FolderComparisonResult, FolderDigest},
    errors::DuplicateError,
    hashing::full_blake3,
    traversal::is_protected_path,
};
use std::{
    collections::HashSet,
    path::{Path, PathBuf},
...
fn digest_folder(path: &Path, exclusions: &[PathBuf]) -> Result<FolderDigest, DuplicateError> {
    let canonical_root = dunce::canonicalize(path)
        .map_err(|error| DuplicateError::InvalidScanSource(error.to_string()))?;
    if is_protected_path(&canonical_root) {
...
pub fn compare_folders(
...
) -> Result<FolderComparisonResult, DuplicateError> {
    if request.paths.len() < 2 {
        return Err(DuplicateError::InvalidScanSource(
            "At least two folders are required for comparison.".into(),
        ));
    }
    ...
    for path in &request.paths {
        folders.push(digest_folder(Path::new(path), &exclusions)?);
    }
    ...
    for left_index in 0..folders.len() {
        for right_index in (left_index + 1)..folders.len() {
            ...
            let classification = if left.digest == right.digest {
                "identical"
            } else if left_only == 0 {
                "right_contains_left"
            } else if right_only == 0 {
                "left_contains_right"
            } else if common > 0 {
                "overlapping"
            } else {
            ...
            comparisons.push(FolderComparison {
