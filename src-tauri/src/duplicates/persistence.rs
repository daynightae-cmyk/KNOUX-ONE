use crate::{duplicates::contracts::{DuplicateScanRequest, DuplicateScanResult}, storage::database};
use rusqlite::{params, Connection};
use tauri::AppHandle;

pub fn persist_scan(app: &AppHandle, request: &DuplicateScanRequest, result: &DuplicateScanResult) -> Result<(), String> {
    let mut connection = database::open(app)?;
    let transaction = connection.transaction().map_err(|error| format!("scan_transaction_failed: {error}"))?;
    ...
    for source in &request.paths { transaction.execute("INSERT INTO duplicate_scan_sources(scan_session_id, path) VALUES (?1, ?2)", params![&result.summary.scan_id, source]).map_err(|error| format!("scan_source_persist_failed: {error}"))?; }
    for exclusion in &request.excluded_paths { transaction.execute("INSERT INTO duplicate_scan_exclusions(scan_session_id, path) VALUES (?1, ?2)", params![&result.summary.scan_id, exclusion]).map_err(|error| format!("scan_exclusion_persist_failed: {error}"))?; }
    ...
    transaction.commit().map_err(|error| format!("scan_commit_failed: {error}"))
}
...
pub fn load_scan_history(connection: &Connection) -> Result<Vec<serde_json::Value>, String> {
    ...
    rows.collect::<Result<Vec<_>, _>>().map_err(|error| format!("scan_history_decode_failed: {error}"))
}

pub fn load_scan_result(connection: &Connection, scan_id: &str) -> Result<DuplicateScanResult, String> {
    ...
    let mut source_statement = connection.prepare("SELECT path FROM duplicate_scan_sources WHERE scan_session_id = ?1 ORDER BY id").map_err(|error| format!("scan_result_sources_prepare_failed: {error}"))?;
    summary.target_folders = source_statement.query_map(params![scan_id], |row| row.get::<_, String>(0)).map_err(|error| format!("scan_result_sources_query_failed: {error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("scan_result_sources_decode_failed: {error}"))?;
    ...
    let group_rows = group_statement.query_map(params![scan_id], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?, row.get::<_, String>(3)?, row.get::<_, String>(4)?, row.get::<_, f64>(5)? as f32, row.get::<_, i64>(6)? != 0, row.get::<_, i64>(7)? as u64))).map_err(|error| format!("scan_result_groups_query_failed: {error}"))?;
    let mut groups = Vec::new();
    for group_row in group_rows {
        let (group_id, mode, category, common_hash, proof_status, confidence, actionable, wasted_size_bytes) = group_row.map_err(|error| format!("scan_result_group_decode_failed: {error}"))?;
        ...
        let files = file_statement.query_map(params![&group_id], |row| Ok(DuplicateFileItem { id: row.get(0)?, path: row.get(1)?, canonical_path: row.get(2)?, name: row.get(3)?, extension: row.get(4)?, size_bytes: row.get::<_, i64>(5)? as u64, modified_time: row.get(6)?, created_time: row.get(7)?, hash: row.get::<_, Option<String>>(8)?.unwrap_or_default(), partial_hash: row.get(9)?, perceptual_hash: row.get(10)?, similarity_score: row.get::<_, Option<f64>>(11)?.map(|value| value as f32), mime_type: row.get(12)?, width: None, height: None, file_identity: row.get(13)?, hard_link_count: row.get::<_, i64>(14)? as u64, is_hard_link_alias: row.get::<_, i64>(15)? != 0, protected_path: row.get::<_, i64>(16)? != 0 })).map_err(|error| format!("scan_result_files_query_failed: {error}"))?.collect::<Result<Vec<_>, _>>().map_err(|error| format!("scan_result_files_decode_failed: {error}"))?;
        groups.push(DuplicateGroup { group_id, mode, category, files, wasted_size_bytes, common_hash, proof_status, confidence, actionable, warnings: Vec::new() });
    }
    summary.duplicate_files_found = groups.iter().map(|group| group.files.len() as u64).sum();
    Ok(DuplicateScanResult { job_id: format!("history:{scan_id}"), groups, summary, warnings })
}
