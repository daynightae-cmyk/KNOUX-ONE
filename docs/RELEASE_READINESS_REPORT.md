...
هذا حكم إنتاجي صادق. أصبح المستودع يملك مسار بناء وحزم وتحديث وإصدار قابل للمراجعة، ومفتاح Tauri محفوظاً خارج Git ومضمناً مفتاحه العام في العميل، وبيئتي نشر مقيدتين بوسوم الإصدار وبمراجعة مطلوبة. لكن لا توجد حتى الآن مادة توقيع Authenticode، ولا إصدار منشور أو اختبار runtime. لذلك لا يجوز وصف أي installer حالي بأنه صالح للتوزيع العام، ولا يجوز نشر `latest.json`، ولا يجوز رفع حالة مركز الإصدارات إلى تحقق runtime.
...
| قنوات `beta` و`stable`    | **PASS — تهيئة فقط**           | ملفات config منفصلة ومولّد `latest.json` حتمي وبيئتا النشر مقيدتان بوسم `v*` مع مراجع مطلوب | لا manifest منشور ولا artifacts release موقعة Authenticode.                                |
...
تحقق الوصول والتنفيذ في 19 أغسطس 2026 من أسماء الإعدادات فقط، من دون قراءة أي قيمة سرية. أنشئت `beta` وتحققت مطابقة `production` للبيئة القائمة، وقيدت كلتاهما بسياسة deploy لوسم `v*` فقط. خُزن السر `TAURI_SIGNING_PRIVATE_KEY` في البيئتين من الملف المحلي خارج Git. أضيف `daynightae-cmyk` كمراجع مطلوب إلى البيئتين مع السماح بالموافقة الذاتية لأنه الحساب الإداري الوحيد. لا توجد شهادة PFX أو بيانات Azure أو متغيرات توقيع Windows.
...
| **BLOCKED-003** | منفذ؛ بيئتا `beta` و`production` مقيدتان بوسم `v*` ومراجع مطلوب           | أضف مراجعاً مستقلاً وفعّل منع self-approval عند توفر متعاون مؤهل.                 |
...
المستودع الآن **مهيأ تقنياً لتوقيع updater وقناتي الإصدار، لكنه ليس بعد منتجاً قابلاً للتوزيع العام**. لم يُنشأ tag أو GitHub Release عمداً، لأن بوابة Authenticode ما زالت بلا شهادة/مسار Azure و`signtool.exe`. قبل أول tag عام، يلزم استكمال هذا البند ثم تنفيذ إصدار tag والتحقق من artifacts والتحديث runtime. راجع [متطلبات المالك المحجوبة](./RELEASE_BLOCKED_REQUIREMENTS.md) لإغلاق البند بالترتيب الصحيح.
