...
**تنفيذ في 19 أغسطس 2026:** أُنشئ زوج مفاتيح Tauri خارج المستودع في ملف حساب المستخدم المحلي، واستُبدلت قيمة `pubkey` في التهيئة بالمفتاح العام المطابق. خُزن المفتاح الخاص في سر `TAURI_SIGNING_PRIVATE_KEY` داخل بيئتي `beta` و`production` من دون عرضه أو إضافته إلى Git. توثيق Tauri يشدد على أن فقد المفتاح الخاص يمنع إصدار تحديثات موثوقة للمستخدمين الذين ثبتوا التطبيق بالفعل. [1]

```powershell
bunx tauri signer generate -w "$env:USERPROFILE\.tauri\knoux-one-updater.key"
```

يبقى على المالك حفظ نسخة احتياطية محكومة للمفتاح الخاص في vault مؤسسي خارج الجهاز، والتأكد من أن الوصول إلى ملف حساب المستخدم المحلي مقيد. لا تضف ملف المفتاح، أو كلمة مروره، أو نسخة منه إلى Git أو Issues أو logs. إن تقرر تدوير المفتاح إلى مفتاح محمي بكلمة مرور قبل الإصدار العام، يجب تحديث `TAURI_SIGNING_PRIVATE_KEY` و`TAURI_SIGNING_PRIVATE_KEY_PASSWORD` في البيئتين و`pubkey` في التطبيق معاً قبل تثبيت أي عميل عام.
