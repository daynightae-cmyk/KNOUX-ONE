...
**الحالة:** منفذ جزئياً؛ يظل الإصدار العام محجوباً حتى يوفّر المالك هوية Authenticode ويعيّن مراجعين مفوضين.
...
| **BLOCKED-003** | منفذ جزئياً | مالك المستودع                       | بيئتا GitHub المحميتان `beta` و`production` | أنشئت `beta` وسياسة الوسم `v*` لكلا البيئتين؛ يلزم تعيين مراجعين مفوضين قبل أول إصدار عام. |
...
**تنفيذ في 19 أغسطس 2026:** أُنشئت بيئة **`beta`**، وتأكدت مطابقة اسم `production` للبيئة القائمة. قُيدت كلتا البيئتين بوسوم `v*` فقط، فلا يمكن لفرع أو PR أو ref آخر أن ينشر عبرهما. يختار workflow البيئة تلقائياً من نوع النسخة: `X.Y.Z-beta.N` يستخدم `beta`، و`X.Y.Z` يستخدم `production`. يجب أن يعيّن المالك مراجعين مفوضين ويمنع self-approval قبل أول إصدار عام، لأن ذلك قرار حوكمة لا يمكن افتراضه نيابةً عنه.
